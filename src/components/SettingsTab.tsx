...
export default function SettingsTab({ user }: Props) {
  ...
  useEffect(() => {
    if (settings) {
      reset({
        name: settings.name || 'Health360',
        tagline: settings.tagline || 'Physiotherapy and Craniosacral Therapy Clinic',
        phone: settings.phone || '8482812859',
        email: settings.email || 'health360vasai@gmail.com',
        address: settings.address || 'Shop No.1, Amardeep Society, Om Nagar, Vasai (W).',
        primaryDoctor: settings.primaryDoctor || 'Dr. Rashmita Karvir Kekre',
        doctorNameCredentials: settings.doctorNameCredentials || 'Dr. Rashmita Karvir Kekre\nB.PTh.(M.I.A.P.)\nBCST',
        logoUrl: settings.logoUrl || '',
        website: settings.website || 'health360vasai@gmail.com',
        upiId: settings.upiId || '8482812859@upi',
